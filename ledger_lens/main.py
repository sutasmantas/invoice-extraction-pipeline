...
from ledger_lens.extractor import extract_document, extract_invoice
from ledger_lens.schemas import Correction, CorrectionRecord, Document, ExportPayload
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    def health() -> dict[str, str]:
        return {"status": "ok", "ocr": "tesseract", "pdf": "pdfium"}
    ...
    @app.get(
        "/api/documents/{document_id}/corrections",
        response_model=list[CorrectionRecord],
    )
    def corrections(document_id: str, request: Request) -> list[CorrectionRecord]:
        try:
            return store(request).corrections(document_id)
        except KeyError as exc:
