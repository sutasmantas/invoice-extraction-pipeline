...
def test_extract_review_correct_export(tmp_path: Path) -> None:
    ...
    with TestClient(create_app(settings)) as client:
        ...
        corrected_again = client.patch(
            f"/api/documents/{document['id']}/fields/vat_id",
            json={"value": "DE278445902"},
        )
        assert corrected_again.status_code == 200

        history = client.get(f"/api/documents/{document['id']}/corrections")
        assert history.status_code == 200
        assert [row["corrected_value"] for row in history.json()] == [
            "DE278445901",
            "DE278445902",
        ]
        ...
        assert exported["data"]["vat_id"] == "DE278445902"
